import type { CellState, Difficulty, WordInfo, WordMembership } from './types.js';
import { BOARD_SIZE, DIFFICULTY_CONFIGS } from './constants.js';
import { WORD_LIST, WORD_SET } from './wordList.js';
import { getWordEmoji } from './emojiHints.js';
...
function buildCrosswordGrid(): { grid: string[][]; placedWords: string[] } {
  ...
  for (const word of wordQueue) {
    ...
    if (placedWords.length >= 50) break;
